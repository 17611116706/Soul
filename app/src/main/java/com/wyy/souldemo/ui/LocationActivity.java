    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.menu_send) {
            ...
            if (ITEM > 0) {
                ...
                intent.putExtra("addres", IAddress);
            } else {
                //直接点击
                intent.putExtra("la", aMap.getMyLocation().getLatitude());
                intent.putExtra("lo", aMap.getMyLocation().getLongitude());
                MapManager.getInstance().poi2address(aMap.getMyLocation().getLatitude(), aMap.getMyLocation().getLongitude(), new MapManager.OnPoi2AddressGeocodeListener() {
                    @Override
                    public void poi2address(String address) {
                        LogUtils.i(address+"999");
